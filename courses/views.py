...
def course_page(request):
    ...
    if user.is_authenticated:
        usercourse_lstdct = UserCourse.objects.filter(user=user, delete__isnull=True).values("course_id", "rating", "progress", "is_paid", "status")
...
def user_courses(request):
    user = request.user
    usercourse_lstdct = UserCourse.objects.filter(user=user, delete__isnull=True).order_by("-created_at").values(
        "course__title", "course__category__title", "course__author__username",
        "created_at", "completed_at", "last_accessed",  "course__description", 
        "rating", "progress", "is_paid", "status", 'course_id')
    return render(
        request,
...
def user2course(request):
    if request.method == "POST":
        ...
        if UserCourse.objects.filter(user=user, course=course, delete__isnull=True).exists():
            defaults = {"delete": timezone.now()}
            UserCourse.objects.update_or_create(user=user, course=course, defaults=defaults)
